...
function addGroupTodoList(listInfo) {
    fetch("http://localhost:5000/groups", {
        ...
        _groupTodoList.push(json);
        addList(json.text, json.checked);
        lengthCheck();
    });
}
    ...
    .then((data) => {
        ...
        for (var i = 0; i < memberNumber; i++) {
            ...
            if(todo_count[i]==0 ){
                ...
                goal_rate.push(success_count[i]/todo_count[i]);
            }  
        }
        var i=0;
        var new_name=[];
        ...
        _content+=`<div id='rate${i}'>${goal_rate[me]}</div></div>`;
        content.innerHTML+=_content;
        var str='rate'+String(i);
        var str1=String(goal_rate[me]*800)+'px';
        ...
        if(new_name.length==1){
            ...
            i++;
            _content=`<div class='item'><div class='name'>${new_name[first_index]}</div>`;
            _content+=`<div id='rate${i}'>${first}</div></div>`;
            content.innerHTML+=_content;
            var str='rate'+String(i);
            var str1=String(first*800)+'px';
            document.getElementById(str).style.width=str1;
            i++;
            _content=`<div class='item'><div class='name'>${new_name[second_index]}</div>`;
            _content+=`<div id='rate${i}'>${second}</div></div>`;
            content.innerHTML+=_content;
            var str='rate'+String(i);
            var str1=String(second*800)+'px';
            document.getElementById(str).style.width=str1;
        }
        }
