...
.then((response)=>{
    ...
    let groupColorArray = originalData.color;
    let profileSrc = originalData.profileImage;
    let groupLinkArray = originalData.pageLink;
    ...
    //프로필 사진 등록
    if(profileSrc != ""){
        profile.setAttribute("src","data:image/png;base64, "+profileSrc);
    }else{
    ...
    if(groupNameArray.length != 0){
        for(let i = 0; i < groupNameArray.length; i++){
            ...
            groupButton.children[i].setAttribute("href",groupLinkArray[i]); //링크 연결
        }
    }
    //버튼 생성
    makeGroupButton(groupNameArray, groupExplainArray, groupNumberArray, groupColorArray);
});
